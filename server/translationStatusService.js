...
const encodeRepo = repo => repo.split("/").map(encodeURIComponent).join("/");
const encodePath = path => path.split("/").map(encodeURIComponent).join("/");

const fetchLatestCommit = async (document, githubToken) => {
...
const fetchRawDocument = async document => {
  const response = await fetch(
...
const prepareMarkdown = value => String(value || "")
  .replace(/```[\s\S]*?```/g, "\n```code block omitted```\n")
...
const validateAnalyses = (value, documents) => {
  ...
  value.analyses.forEach(analysis => {
    if (!expectedIds.has(analysis.id) || !STATUS_TEXT[analysis.status]) return;
...
const analyzeTranslationStatus = async ({ apiKey, githubToken, model, force }) => {
  ...
  const commitPairs = await Promise.all(documents.map(async document => {
    ...
    return [document.analysisId, { source, translation }];
  }));
