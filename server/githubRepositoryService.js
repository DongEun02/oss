...
const isMaintainerAssociation = association => MAINTAINER_ASSOCIATIONS.has(association || "");

const isBotLogin = login => BOT_LOGIN_PATTERN.test(login || "");

const firstMaintainerResponseAt = pullRequest => {
  const responseDates = [
...
export const summarizeContributorFriendliness = (
  ...
  const fetchedCreatedDates = (pullRequests || [])
    .map(pullRequest => Date.parse(pullRequest.createdAt || pullRequest.created_at || ""))
    .filter(Number.isFinite);
  const sampleLimited = hasMorePullRequests
    && fetchedCreatedDates.length > 0
    && Math.min(...fetchedCreatedDates) >= windowStart;
  const recentExternalPullRequests = (pullRequests || []).filter(pullRequest => {
  ...
  const firstResponseHours = recentExternalPullRequests.flatMap(pullRequest => {
  ...
  const mergedPullRequestCount = recentExternalPullRequests.filter(
    pullRequest => pullRequest.mergedAt || pullRequest.merged_at
  ).length;
  const stalePullRequestCount = recentExternalPullRequests.filter(pullRequest => {
...
const fetchContributorPullRequestsWithGraphql = async (fullName, githubToken) => {
...
const fetchContributorPullRequestsWithRest = async (fullName, githubToken) => {
...
export const fetchRepositoryContributorFriendliness = async (repository, githubToken) => {
