...
const extractJsonObject = (value: any) => {
...
const readErrorMessage = async (response: any) => {
  try {
...
export const generateNvidiaJson = async ({
  ...
  try {
    ...
    if (!response.ok) {
      ...
      const error = Object.assign(new Error(`NVIDIA_HTTP_${response.status}`), { providerMessage });
      throw error;
    }
