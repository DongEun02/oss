const fallbackRepositoryAvatar = (repository: any) => {
  const owner = String(repository || "").split("/")[0];
  return owner ? `https://github.com/${owner}.png?size=320` : "";
};

const normalizeImportedIssue = ({ issue, repository }: any) => {
  ...
  return {
    ...
    languageTags,
    techs: [...new Set([
      ...labels.map((label: any) => label.name),
      ...repositoryTopics,
      ...languageTags
...
export const fetchGithubIssueByUrl = async (
  issueUrl: any,
